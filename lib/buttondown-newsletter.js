...
export async function sendWelcomeEmail(apiKey, email, html) {
  ...
  // Step 2: Send the draft to this subscriber
  const sendRes = await fetch(
    `${BUTTONDOWN_API_BASE}/emails/${draft.id}/send-draft`,
    {
      method: "POST",
      headers: {
        Authorization: `Token ${apiKey}`,
  ...
  // Step 3: Clean up the draft regardless of send outcome
  fetch(`${BUTTONDOWN_API_BASE}/emails/${draft.id}`, {
    method: "DELETE",
