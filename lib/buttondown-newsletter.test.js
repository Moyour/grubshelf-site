...
describe("sendWelcomeEmail", () => {
  ...
  it("creates a draft then sends it to the subscriber", async () => {
    ...
    // Verify send-draft call targets the subscriber
    expect(mockFetch.mock.calls[1][0]).toContain("emails/email-123/send-draft");
    const sendBody = JSON.parse(mockFetch.mock.calls[1][1].body);
    expect(sendBody.subscriber_emails).toEqual(["test@example.com"]);
  });
  ...
  it("returns failure when send-draft fails", async () => {
    const mockFetch = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ id: "email-123" }),
